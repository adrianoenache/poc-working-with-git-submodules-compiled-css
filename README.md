# Proof of concept (POC) of working with Git submodules - Compiled CSS
...
This repository is responsible for compiling Stylus source files into a distributed CSS file and exporting the Stylus core config. The compiled output is published to [`poc-working-with-git-submodules-packages-for-distribution`](https://github.com/adrianoenache/poc-working-with-git-submodules-packages-for-distribution) via a git submodule linked at `distribution/`.
...
| Directory | Purpose |
|---|---|
| `stylus/` | Stylus CSS source files (atomic design structure) |
| `stylus/entry/` | Entry point file (`main.styl`) — imports the full Stylus source tree |
| `distribution/` | Git submodule pointing to `poc-working-with-git-submodules-packages-for-distribution`; compiled CSS is generated here |
...
```bash
git clone --recurse-submodules git@github.com:adrianoenache/poc-working-with-git-submodules-compiled-css.git
```

If you clone the repository without the `--recurse-submodules` parameter:

```bash
git clone git@github.com:adrianoenache/poc-working-with-git-submodules-compiled-css.git
```
...
This command runs `npm install`, sets up Husky pre-commit hooks and **starts the CSS watcher** — the terminal will remain blocked while the watcher is running. To install only, without starting the watcher:
...
This command removes `node_modules`, clears the npm cache, reinstalls all dependencies, runs `npm audit fix` and lists outdated dependencies (`npm outdated`). At the end, the CSS watcher will start and **the terminal will remain blocked**.
...
|---|---|
| `npm run start` | Start CSS watcher (alias for `css-watch`) |
| `npm run css` | Lint + compile CSS (production) |
...
| `npm run export-stylus-core` | Copy Stylus core config into `distribution/stylus-core-config/` |
| `npm run cleanup-export-stylus-core` | Remove the exported Stylus core config from `distribution/` |
| `npm run npm-reinstall` | Removes `node_modules`, clears the npm cache, reinstalls all dependencies, runs `npm audit fix`, lists outdated dependencies and starts the CSS watcher |
...
- [Stylus — Expressive, dynamic, robust CSS](https://stylus-lang.com/)
